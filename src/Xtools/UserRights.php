...
class UserRights extends Model
{
    ...
    public function getRightsStates()
    {
        ...
        foreach (['local', 'global'] as $type) {
            ...
            $this->rightsNames = array_unique(array_merge(
                $this->rightsNames, $added, $former
            ));
    ...
    {
        if (isset($this->localizedRightsNames)) {
            return $this->localizedRightsNames;
        }

        $rightsToCheck = array_unique(array_merge(
            $this->user->getUserRights($this->project),
            $this->user->getGlobalUserRights($this->project),
            $this->rightsNames
        ));
